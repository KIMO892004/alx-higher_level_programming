#!/usr/bin/python3

"""Defining classes for a singly-linked list."""


class Node:
    """Representing a node in a singly-linked list."""

    def __init__(self, data, next_node=None):
        """Initializing a new Node.
        Args:
            data (int): The data of the new Node.
            next_node (Node): The next node of the new Node.
        """
        self.data = data
        self.next_node = next_node

    @property
    def data(self):
        """Get the data of the Node."""
        return (self.__data)

    @data.setter
    def data(self, thevalue):
        if not isinstance(thevalue, int):
            raise TypeError("data must be an integer")
        self.__data = thevalue

    @property
    def next_node(self):
        """Get the next_node of the Node."""
        return (self.__next_node)

    @next_node.setter
    def next_node(self, thevalue):
        if not isinstance(thevalue, Node) and thevalue is not None:
            raise TypeError("next_node must be a Node object")
        self.__next_node = thevalue


class SinglyLinkedList:
    """Representing a singly-linked list."""

    def __init__(self):
        """Initalizing a new SinglyLinkedList."""
        self.__head = None

    def sorted_insert(self, thevalue):
        """Inserting a new Node to the SinglyLinkedList.
        The node is inserted into the list at the correct
        ordered numerical position.
        Args:
            value (Node): The new Node to insert.
        """
        new = Node(thevalue)
        if self.__head is None:
            new.next_node = None
            self.__head = new
        elif self.__head.data > thevalue:
            new.next_node = self.__head
            self.__head = new
        else:
            tmp = self.__head
            while (tmp.next_node is not None and
                    tmp.next_node.data < thevalue):
                tmp = tmp.next_node
            new.next_node = tmp.next_node
            tmp.next_node = new

    def __str__(self):
        """Defining the print() representation of a SinglyLinkedList."""
        values = []
        tmp = self.__head
        while tmp is not None:
            values.append(str(tmp.data))
            tmp = tmp.next_node
        return ('\n'.join(values))
